...
//            });
            ...
            var textArrayItem = paper.text(xArray[i], yArray[i] + 45, titleSub).attr({
                "fill": '#fff',
                "font-family": '微软雅黑',
                "font-size": "14",
                title: titleArray[i],
                opacity: 0,
                cursor: "pointer"
            }).data("index", i).animate({opacity: 1}, 700, "ease").click(function (e) {
                nodeClick(e, this)
            });

            var rectArrayItem = paper.rect(xArray[i] - 12, yArray[i] + 3, 0, 0).attr({
                fill: "#389b9f",
                opacity: 0,
                transform: "r45",
                width: 24,
                ...
                opacity: 0,
                cursor: "pointer"
            }).data("index", i).animate({"opacity": 1, transform: "r45"}, 700, "ease").click(function (e) {
                nodeClick(e, this)
            });
            var hotArrayItem = paper.text(xArray[i], yArray[i] + 15, scoreArray[i]).attr({
                "fill": '#fff',
                "font-family": '微软雅黑',
                "font-size": "16",
                opacity: 0,
        ...
        success: function (data) {
            ...
            if (data && data.education.length > 0) {
                ...
                $.each(data.education, function (index, item) {
                    educationOption.legend.data.push({
                        name: item.name + " " + item.value.toFixed(2) + "%",
                        icon: "circle"
                    });
                    var tempItem = JSON.stringify(item);
